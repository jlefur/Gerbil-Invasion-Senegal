...
Globals [
  ;;ENVIRONMENT_Global variables
  raster-data;; input raster
  vecteur-data;; input vector file
  list-rainy-seasons;;list of months and duration of rainy seasons (2 values per year)
  list-nb-month-pregnancy;; Gestation period depends on rainfall. There is a number of months per rainy year.
  list-coordinates-x;; list of x-coordinates for all populations
  list-coordinates-y;; list of y-coordinates for all populations

  ;;GERBILS_Global variables

  monthly-distance-per-month;; distance travelled per month by propagules (nb patches)
  ...
  list-%pregnants;;% of pregnant females per month from start of the breeding period (August) until propagule departure
  %-pregnants-month;;% of pregnant females per month (number used in the ‘reproduce’ procedure)
  rotate;; possible amplitude of a random direction (340°, i.e. -170;+170 degrees; <360° to avoid staying in one place)
  initial-position-x-E;; eastern initial position
  initial-position-x-W;; western initial position
  initial-position-y-S;; southern initial position
  initial-position-y-N;; northern initial position

  ;; TEMPORALITE
  start-cycle;; allows to enter the reproduction phase
  breeding-season;; indicates whether it's the breeding season
  month-start-breeding-season;; 1st month of breeding season
  month-end-rainy-season;; month at the end of the rainy season (1st month without rain)
  duration-breeding-after-rainy-season;; duration of reproduction (number of months) after the end of the rainy season
  month-end-breeding-season;; end of breeding period
  month-end-breeding-season+1;; month following the end of breeding period
  month-end-cycle;; useful for determining the end of cycle date (1st month after the departure of the last propagules of the year)

  ;; RESULTATS
  expansion-east;; maximum horizontal distance to east (= max(end position) - init position)(the scene's origin is bottom-left.)
  expansion-west;; horizontal distance to west (= min(position-finale) - position init)(the scene's origin is bottom-left.)
  expansion-south;; maximum vertical distance to south (= position-init - min(position-finale))(the scene's origin is bottom-left.)
  expansion-north;; maximum vertical distance (= position-init - min(position-finale))(the scene's origin is bottom-left.)
  ]

Patches-own [
  env-limits;;country limits
  env-river;; Senegal river
  env-land-cover;; class Synmap reclassified
  env-ndvi;; Ndvi value (NOAA files - from 8km->4km resampling)
  env-rainfall;; monthly rainfall qty (mm)(GPCC data from January 1982 ro December 1998 - TRMM 3B42 from January 1999 to December 2013)
  env-pedo;6 soil classes: 1 clay; 2 other; 3 sand; 4 s-a; 5 NR; 6 sea-water (from IRD 1971 soil map)
  env-quality-habitat;;habitat quality suitable for settlement
  env-quality-habitat-ndvi; 1 if favorable ndvi
  env-quality-habitat-land-cover;  1 if landcover favorable landcover
  env-quality-habitat-pedo;   1 if pedology favorable pedology
  env-occupied_patch;   1 if a population is installed on the patch, 0 otherwise
]

Breed [gerbils Gerbil]
gerbils-own [
  age ;; number of months
  IsSettled?  ;; to determine whether a population has arrived or not
  isMerged? ;; merge indicator in procedure gerbils_merge_on_patch (true: population stays and merges; false: population is merged with another population)
  nb-males
  nb-females
  nb-youngsters
  list-females-pregnants
  ...
  ;; The first value (item0) is used to calculate the number of young at the time of departure. (variable %-pregnants-month)
  status ;; reproduction (rainy season), sexual rest (dry season), exploration (propagules status)
]
...
ifelse nb-populations != 0  [;;  at least one population
 ;; TO REPRODUCE
  ...
  if month-num  = month-start-breeding-season [
    set start-cycle true
    set breeding-season true
    ask gerbils with [status != "exploration"][;; mobile propagules do not reproduce; their reproductive phase begins only after installation.
    set status "reproduction"
    set color red
    ]]
  if month-num  = month-end-cycle and month-num < month-start-breeding-season [;;cycle: beginning of reproduction (in August) until the end of the young's departure (before August of the following year)
    set start-cycle false]
  if month-num  = month-end-breeding-season[;;period-reprod: stops at the end of reproduction
    set breeding-season false]

     ;;****************reproduction**********************************************************************************
 if start-cycle [
   ask gerbils with [status != "exploration"][;; mobile propagules do not reproduce
  ...
  ;; distance traveled by propagules (function of patch size, here 4 km*4km)
    set monthly-distance-per-month round(monthly-distance / 3885);; "monthly-distance” interface item _ pixels are rather 3.80*3.97, i.e. an average of 3.885...)
    ask gerbils with [status = "exploration"] [
...
to  setup-variables
  ...
  set month-num 0
  set start-cycle false
  set  list-coordinates-x []
  set  list-coordinates-y []
  ...
  set list-rainy-seasons[8 11 8 11 8 11 8 11 8 11 8 11 8 11 8 11 8 11 8 11 8 12 8 12 8 11 8 11 8 11 8 11 8 11 8 11 8 12 8 12 8 11 8 11 8 12 8 11 8 11 8 12 8 11 8 12 8 10 8 11 8 11 8 11]
  ;; nb of months with gestation after the end of the rainy season
  set list-nb-month-pregnancy [2 1 2 2 2 2 3 3 2 2 2 3 1 3 2 2 2 4 3 3 2 4 3 4 3 2 4 4 5 2 4 4]
  ;; % pregnant females from start of breeding season (August) (see article)
  set list-%pregnants [6 36 34 27 14 17 31 6 14 10 0 3]
end
...
to proc-env-landcover  ;; SYNMAP grid
 ...
 resize-world 0 (gis:width-of raster-data - 1) 0 (gis:height-of raster-data - 1)
 gis:set-world-envelope gis:envelope-of raster-data
 gis:apply-raster raster-data env-land-cover
 ;; habitat quality - landcover
 ask patches with [env-land-cover = 1 or env-land-cover = 16] [set env-quality-habitat-land-cover 3];;water, buildings
 ask patches with [env-land-cover = 8 or env-land-cover = 11 or env-land-cover = 14 ] [set env-quality-habitat-land-cover 1 ];;
 ask patches with [ (env-land-cover >= 2 and env-land-cover <= 7) or env-land-cover = 9 or  env-land-cover = 10 or env-land-cover = 12 or  env-land-cover = 13 or  env-land-cover = 15] [set env-quality-habitat-land-cover 2]
end
...
to proc-env-pedo ;; pedology
 ...
 gis:apply-raster raster-data env-pedo
  ...
  ask patches with [env-pedo = 6] [set env-quality-habitat-pedo 3];;water
  ask patches with [env-pedo = 2  or env-pedo = 3 or env-pedo = 4] [set env-quality-habitat-pedo 1 ];;other (2) or sand (3) or sand-clay (4)
  ask patches with [env-pedo = 1  or env-pedo = 5] [set env-quality-habitat-pedo 3 ];;clay (1) ou undetermined (5)
  ask patches with [env-pedo < 1  or env-pedo > 6] [set env-quality-habitat-pedo 3];;pedology not specified
end
 ...
 ;; habitat quality - NDVI
  ask patches [
   ifelse month-num >= 11 or month-num <= 6 [;;  quality 1 if ndvi between 19 and 24 in dry season
      ifelse env-ndvi >= NDVI_min and env-ndvi <= NDVI_max [set env-quality-habitat-ndvi 1 ][set env-quality-habitat-ndvi 3]]
    [set env-quality-habitat-ndvi 1;; in the wet season, all ndvi are accepted; see article
 ]]
end
...
to  proc-env-habitat  ;; habitat quality (1, 2 or 3)
  if habitat-ndvi? [
     ask patches with [env-limits != 0][
       ifelse env-quality-habitat-ndvi = 1 [set env-quality-habitat 1] [set env-quality-habitat 3]
   ]]
  if habitat-land-cover? [
     ask patches with [env-quality-habitat-land-cover = 1 and env-limits != 0] [set env-quality-habitat 1]
     ask patches with [env-quality-habitat-land-cover = 2 and env-limits != 0] [set env-quality-habitat 2]
     ask patches with [env-quality-habitat-land-cover = 3 and env-limits != 0] [set env-quality-habitat 3]
   ]
 if habitat-pedo? [
     ask patches with [env-quality-habitat-pedo = 1 and env-limits != 0] [set env-quality-habitat 1]
     ask patches with [env-quality-habitat-pedo = 3 and env-limits != 0] [set env-quality-habitat 3]
   ]
 if habitat-lc-pedo? [
      ask patches with [env-limits != 0][
        ifelse env-quality-habitat-land-cover = 3 [;; cases where the land-cover is not habitable (water, buildings)
        set env-quality-habitat 3] [
        ifelse env-quality-habitat-land-cover = 1 and env-quality-habitat-pedo = 1 [set env-quality-habitat 1] [set env-quality-habitat 3]
         ]]]
...
if habitat-lc-ndvi? [
      ask patches with [env-limits != 0][
        ifelse env-quality-habitat-land-cover = 3 [;; cases where the land-cover is not habitable (water, buildings)
        set env-quality-habitat 3] [
        ifelse env-quality-habitat-land-cover = 1 and env-quality-habitat-ndvi = 1 [set env-quality-habitat 1] [set env-quality-habitat 3]
          ]]]
if habitat-pedo-ndvi?  [
        ask patches with [env-limits != 0][
        ifelse env-quality-habitat-land-cover = 3 [;; cases where the land-cover is not habitable (water, buildings)
        set env-quality-habitat 3] [
        ifelse env-quality-habitat-pedo = 1 and env-quality-habitat-ndvi = 1 [set env-quality-habitat 1] [set env-quality-habitat 3]
         ]]]
...
if habitat-ndvi-landcover-pedo? [
     ...
     ask patches with [env-limits != 0][
        ifelse env-quality-habitat-land-cover = 3 [;; cases where the land-cover is not habitable (water, buildings)
        set env-quality-habitat 3] [
     ifelse env-quality-habitat-land-cover = 1 and env-quality-habitat-ndvi = 1 and env-quality-habitat-pedo = 1 [set env-quality-habitat 1] [set env-quality-habitat 3]
      ]]]
end
...
to proc-env-rainfall-monthly  ;; Monthly loading of rainfall grid - GPCC (january 82-december 97) - TRMM 3B42 (january 98 - december 2013)
   ...
   resize-world 0 (gis:width-of raster-data - 1) 0 (gis:height-of raster-data - 1)
   gis:set-world-envelope gis:envelope-of raster-data
   gis:apply-raster raster-data env-rainfall
end
...
to proc-env-rainfall-annual;;  loading of rainfall data and end of breeding period
  ...
  set month-start-breeding-season first  list-rainy-seasons;; August : first month of procreation
  set list-rainy-seasons remove-item 0  list-rainy-seasons;; update start of rainy season for following year  (1er item)
  set month-end-rainy-season first  list-rainy-seasons
  set list-rainy-seasons remove-item 0  list-rainy-seasons;; update end of rainy season for following year  (2nd item)
  ...
  set duration-breeding-after-rainy-season first list-nb-month-pregnancy
  set list-nb-month-pregnancy remove-item 0 list-nb-month-pregnancy;; update “list-nb-month-pregnancy” list for next year (1st item)
  set month-end-breeding-season  month-end-rainy-season + duration-breeding-after-rainy-season

  set month-end-breeding-season  month-end-breeding-season mod 12
  set month-end-breeding-season+1  (month-end-breeding-season  + 1) mod 12
  set month-end-cycle   (month-end-breeding-season + 3)  mod 12
  if month-end-breeding-season = 0 [;; when the month of end of reproduction is December
    set month-end-breeding-season 12]
  if month-end-cycle = 0 [;;when the end-of-cycle month is December
    set month-end-cycle 12]
end
...
to proc-gerbils-init-position[position-x position-y increment-x increment-y];;initial population loading according to x y position and horizontal and vertical spacing
  ...
  create-gerbils nb-populations[
      ...
      set list-females-pregnants [0 0 0 0]
      set status "sexual rest"
      set age 1
      set color black
  ...
  set list-coordinates-x lput position-x list-coordinates-x;; all positions x
  set list-coordinates-y lput position-y list-coordinates-y];; all positions y
  set initial-position-x-E max list-coordinates-x;; initial-position-x-E calculates expansion from the easternmost point
  set initial-position-x-W min list-coordinates-x;; initial-position-x-W calculates expansion from the westernmost  point
  set initial-position-y-S min list-coordinates-y;; initial-position-y-S calculates expansion from the southernmost  point
  set initial-position-y-N max list-coordinates-y;; initial-position-y-N calculates expansion from the northernmost   point
end
...
to proc-gerbils-reproduce
  ...
  if not breeding-season or status = "sexual rest"[;; in this case, there are no pregnant gerbils
               set list-females-pregnants lput 0 list-females-pregnants
               set status "sexual rest"
               set color yellow
               ]
  set list-females-pregnants remove-item 0 list-females-pregnants;;list update: after adding the month's value, we remove the oldest (item 0).
...
if month-num = 9 [
    if item 3 list-females-pregnants < item 2 list-females-pregnants [;; nb September < nb August
    set list-females-pregnants replace-item  3 list-females-pregnants  item 2 list-females-pregnants ; then September = August
  ]]
if month-num = 10 [
  if item 3 list-females-pregnants > item 2 list-females-pregnants [;; nb October > nb September
  set list-females-pregnants replace-item  3 list-females-pregnants  item 2 list-females-pregnants ; then October = September
  ]]

;; Number of young (based on 1st element of list of gestating females and litter-size reproduction rate)
          let a first list-females-pregnants
          set nb-youngsters ceiling(litter-size * a)
          if (nb-youngsters > 0 and nb-youngsters < 1)[set nb-youngsters 1];; no population fraction!
end
...
to proc-gerbils-propagules-departures
  ...
  ask gerbils with [nb-youngsters != 0] [;;population is created as soon as there are young people (i.e. over 2 months)
    ;; a new gerbil population is born; it inherits its parents' characteristics
        hatch-gerbils 1  [;;new propagule population
           ...
           set nb-females ceiling((nb-youngsters / 2) * %-propagules)
           set nb-males ceiling((nb-youngsters / 2) * %-propagules)
    ...
    ;; case of young gerbils that don't leave : settled population grows
         set nb-males ceiling(nb-males + (nb-youngsters / 2) * (1 - %-propagules))
         set nb-females ceiling(nb-females + (nb-youngsters / 2) * (1 - %-propagules))
         set nb-youngsters 0
         if (nb-males > 0 and nb-males < 1) or (nb-females > 0 and nb-females < 1) [
            set nb-females 1 set nb-males 1];; no population fraction!
         ...
         if month-num !=  month-end-breeding-season and status = "reproduction"[
         set list-females-pregnants remove-item 3 list-females-pregnants]
...
to proc-gerbils-to-move
  ...
  ;; The proc-gerbils-contour obstacle procedure prevents a population from going to sea!
   ...
   let HabitatLePlusProche min-one-of patches with [env-quality-habitat = 1]  [distance myself];; habitat le plus proche
 ...
 if patch-ahead monthly-distance-per-month != nobody or patch-ahead distance habitatLePlusProche != nobody[

;; First advance
     ifelse age = 0 [;; first time, young propagules are advanced by distance covered
       proc-gerbils-avoid_obstacle
       fd monthly-distance-per-month;;
       ask patch-here [ifelse env-quality-habitat = 1 [set env-occupied_patch 1][set env-occupied_patch 0]]
       ifelse env-occupied_patch = 1 [set IsSettled?  true][set IsSettled?  false]
      ][
...
;; Movement
     ...
     ifelse env-occupied_patch = 1 [set IsSettled?  true][set IsSettled?  false]
   if not IsSettled?  [
      ifelse distance habitatLePlusProche <= monthly-distance-per-month [;; the nearest preferential habitat is accessible (less than monthly-distance-per-month)
            ...
            if env-quality-habitat = 1 [set IsSettled?  true
              ask  patch-here [set env-occupied_patch 1
   ]]][
 ...
 if IsSettled?  [;; in this case, the propagule population is settled ; status = reproduction if it's the breeding season, otherwise status = exual rest.
      ifelse breeding-season [
        set status "reproduction"
        set color red
   ][
        set status "sexual rest"
...
to proc-gerbils-avoid_obstacle
  ...
  set monthly-distance-per-month monthly-distance / 3885;; each pixel is almost 4 km on a side
  let habitatLePlusProche min-one-of patches with [env-quality-habitat-ndvi = 1]  [distance myself]
  let distance-habitat-le-plus-proche distance habitatLePlusProche
  set list-2-distances lput monthly-distance-per-month list-2-distances
...
to proc-gerbils-merge_on_patch;;this procedure tests whether there are several populations on a patch.
 ...
 ask gerbils-here with [not isMerged?];;
    [set  nb-tot-females  nb-tot-females + nb-females
     ...
     set nb-tot-jeunes  nb-tot-jeunes +  nb-youngsters
     set nb-females-gest1 nb-females-gest1 + item 0 list-females-pregnants
     set nb-females-gest2 nb-females-gest2 + item 1 list-females-pregnants
     set nb-females-gest3 nb-females-gest3 + item 2 list-females-pregnants
     set nb-females-gest4 nb-females-gest4 + item 3 list-females-pregnants
     set age round(age * (nb-females + nb-males + nb-youngsters))
     set age-total age-total + age

     ifelse status = "reproduction" [set status-population "reproduction"][
       ifelse status = "sexual rest" [set status-population "sexual rest"][
       ]]
 ...
 set nb-youngsters nb-youngsters + nb-tot-jeunes
 set nb-females-gest1 nb-females-gest1 + item 0 list-females-pregnants
 set nb-females-gest2 nb-females-gest2 + item 1 list-females-pregnants
 set nb-females-gest3 nb-females-gest3 + item 2 list-females-pregnants
 set nb-females-gest4 nb-females-gest4 + item 3 list-females-pregnants
 let list1 replace-item  0 list-females-pregnants nb-females-gest1
 let list2 replace-item  1 list1 nb-females-gest2
 let list3 replace-item 2 list2 nb-females-gest3
 set list-females-pregnants replace-item  3 list3 nb-females-gest4
 set nb-females-gest1 0
 set nb-females-gest2 0
 set nb-females-gest3 0
 set nb-females-gest4 0
 set age round(( age + age-total ) / (nb-males + nb-females + nb-youngsters));; age taking into account population size
 set status status-population
 ifelse status = "reproduction" [set color red][ifelse status = "sexual rest" [set color yellow][set color blue]]
 set isMerged? false
 ]]
end
...
to proc-gerbils-die;;each month, the numbers of each population take into account the survival rate
  ask gerbils[
   ...
   set nb-youngsters round(survival-rate * nb-youngsters)
   ]
...
to utility-display-data;  this utility displays environmental data and preferential habitats according to the interface choices.
  ...
  ask patches with [env-ndvi > -1 and env-ndvi < -0.076] [set pcolor sky + 3];blue ; water
   ...
   if Rainfall? [
    ask patches with [env-rainfall = 0 and  env-limits != 0] [set pcolor 9.9];;white
    ask patches with [env-rainfall >= 1 and env-rainfall <= 9 and env-limits != 0] [set pcolor 48]
    ask patches with [env-rainfall = 10 and env-land-cover != 1 and env-limits != 0] [set pcolor 47];;yellow
    ask patches with [env-rainfall >= 11 and env-rainfall <= 19 and env-limits != 0] [set pcolor 46]
    ask patches with [env-rainfall >= 20 and env-rainfall <= 30 and env-limits != 0] [set pcolor 45];;yellow
    ask patches with [env-rainfall >= 31 and env-rainfall <= 39 and env-limits != 0] [set pcolor 44]
    ask patches with [env-rainfall >= 40 and env-rainfall <= 80 and env-limits != 0] [set pcolor 43];;yellow
    ask patches with [env-rainfall >= 81 and env-rainfall <= 89 and env-limits != 0] [set pcolor 58]
    ask patches with [env-rainfall >= 90 and env-rainfall <= 140 and env-limits != 0] [set pcolor 55];;green
    ask patches with [env-rainfall >= 141 and env-rainfall <= 149 and env-limits != 0] [set pcolor 53]
    ask patches with [env-rainfall >= 150 and env-rainfall <= 200  and env-limits != 0] [set pcolor 52];;green
    ask patches with [env-rainfall >= 201 and env-rainfall <= 209  and env-limits != 0] [set pcolor 73]
    ask patches with [env-rainfall >= 210 and env-rainfall <= 260 and env-limits != 0] [set pcolor 93];;blue
    ask patches with [env-rainfall >= 261 and env-rainfall <= 269 and env-limits != 0] [set pcolor 92]
    ask patches with [env-rainfall >= 270 and env-rainfall != 65535 and env-limits != 0] [set pcolor 112];;purple missing values
    ask patches with [(env-rainfall = -999 or env-rainfall = 65535) and  env-limits != 0] [user-message ("missing data rainfall")]
 ...
 if Landcover? [;;  SYNMAP grid display reclassified into 16 classes
   ask patches with [env-land-cover = 1] [set pcolor blue + 3];;eau
   ask patches with [env-land-cover = 2] [set pcolor brown - 3]
   ask patches with [env-land-cover = 3] [set pcolor brown - 2]
   ask patches with [env-land-cover = 4] [set pcolor brown - 1]
   ask patches with [env-land-cover = 5] [set pcolor brown]
   ask patches with [env-land-cover = 6] [set pcolor brown + 1]
   ask patches with [env-land-cover = 7] [set pcolor green - 3]
   ask patches with [env-land-cover = 8] [set pcolor green - 2]
   ask patches with [env-land-cover = 9] [set pcolor green - 1]
   ask patches with [env-land-cover = 10] [set pcolor green]
   ask patches with [env-land-cover = 11] [set pcolor lime - 2]
   ask patches with [env-land-cover = 12] [set pcolor lime - 1]
   ask patches with [env-land-cover = 13] [set pcolor lime]
   ask patches with [env-land-cover = 14] [set pcolor lime + 1]
   ask patches with [env-land-cover = 15] [set pcolor yellow + 4]
   ask patches with [env-land-cover = 16] [set pcolor grey]
   ask patches with [env-land-cover = 16] [set pcolor white]
     ]
...
if display-habitat? [
  ;; Preferential habitat display (1, 2 ou 3)
    ask patches with [env-quality-habitat = 1 and env-limits != 0] [set pcolor 65 ]
    ask patches with [env-quality-habitat = 2 and env-limits != 0] [set pcolor 67 ]
    ask patches with [env-quality-habitat = 3 and env-limits != 0] [set pcolor white]
    ask patches with [env-quality-habitat != 1 and env-quality-habitat != 2 and env-quality-habitat != 3 and env-limits != 0] [set pcolor red
...
to utility-pregnant_females;; this utility calculates the number of pregnant females according to month
 if month-num !=  month-end-breeding-season and status = "reproduction"[
    ifelse month-num = 8 [;; August - beginning of breeding season
     set  %-pregnants-month  item 0 list-%pregnants ][
        ifelse month-num = 9 [
           set  %-pregnants-month  item 1 list-%pregnants][
         ifelse month-num = 10 [
           set  %-pregnants-month  item 2 list-%pregnants][
         ifelse month-num = 11 [
           set  %-pregnants-month  item 3 list-%pregnants][
         ifelse month-num = 12 [
           set  %-pregnants-month  item 4 list-%pregnants][
         ifelse month-num = 1 [
           set  %-pregnants-month  item 5 list-%pregnants][
         ifelse month-num = 2 [
           set  %-pregnants-month  item 6 list-%pregnants][
         ifelse month-num = 3 [
           set  %-pregnants-month  item 7 list-%pregnants][
         ifelse month-num = 4 [
           set  %-pregnants-month  item 8 list-%pregnants][
         ifelse month-num = 5 [
           set  %-pregnants-month  item 9 list-%pregnants][
         ifelse month-num = 6 [
           set  %-pregnants-month  item 10 list-%pregnants][
        ifelse month-num = 7[
           set  %-pregnants-month  item 11 list-%pregnants][
            user-message (word "error")
    ]]]]]]]]]]]]
 set %-pregnants-month  %-pregnants-month * nb-females / 100
 if (%-pregnants-month > 0 and  %-pregnants-month  < 1) [set %-pregnants-month  1];;no fraction of individuals!
 set list-females-pregnants lput ceiling(%-pregnants-month) list-females-pregnants
 ]
end
...
to utilitaire-expansion;;this utility calculates each month in meters, the 4 expansions south, north, east and west according to the initial position and the position at the end of the step. ask gerbils[
   ask gerbils[
     set list-coordinates-x lput xcor list-coordinates-x;; all positions at x
     set list-coordinates-y lput ycor list-coordinates-y;; all positions at y
  ]

  set expansion-east max list-coordinates-x - initial-position-x-E
  set expansion-west initial-position-x-W - min list-coordinates-x;;  min because the scene originates from bottom-left.
  set expansion-south initial-position-y-S - min list-coordinates-y;; min because the scene originates from bottom-left.
  set expansion-north max list-coordinates-y - initial-position-y-N
  set expansion-east expansion-east * 3885;;  expansion in meters (not number of patches)
  set expansion-west expansion-west * 3885;;  expansion in meters (not number of patches)
  set expansion-south expansion-south * 3885;; expansion in meters (not number of patches)
  set expansion-north expansion-north * 3885;; expansion in meters (not number of patches)
end
...
This model simulates the expansion of the Nigerian gerbil over 3 decades, from 1982 to 2013. 30 years of field observations show an increase of around 200 kilometers, from the north of Senegal in the 1980s to the latitude of Dakar in the 2010s. The factors substrate (density of sand), vegetation (surface covered in herbaceous plants), rainfall and vegetation index are crucial in explaining the expansion of this species. 
For each month from January 1982 to December 2013, the model shows in green the location of these preferential habitats and the movement of populations seeking to settle there. Population status is displayed in three colours: breeding (red), sexual rest (yellow) and exploration (blue).

## HOW IT WORKS
Every year, some of the propagules leave the burrow and look for a favourable place to settle. This preferential habitat meets favourable environmental criteria in terms of soil type, land-cover and ndvi. Once established, this population will begin an sexuel rest and reproduction cycle, which in turn will generate other populations of propagules responsible for the species' expansion.
...
- The third part specifies the gerbil population parameters. A survival rate of 0.75, a litter size of 3.4 and a % of propagules of 50% with an introduction to the north or north-north-east is the most realistic configuration, given current knowledge on the subject.

At the end of a simulation lasting 3 decades, we obtain an expansion of the gerbil from its point of departure in four directions: south, east, west and north. 
